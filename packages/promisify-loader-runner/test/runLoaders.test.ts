...
import { getContext, runLoaders } from '../../webpack-build-utils/src'

const fixtures = path.resolve(__dirname, 'fixtures')

function runLoadersPromise(options: any) {
...
describe.skip('runLoaders', () => {
  it('should process only a resource', async () => {
  ...
  it('should process raw loaders', (done) => {
    runLoaders(
      ...
      (err, result) => {
        if (err) { return done(err) }
        result.result[0].toString('utf-8').should.be.eql('efbbbf62c3b66d﻿böm')
        done()
      },
    )
  })
    ...
    runLoaders(
      ...
      (err, result) => {
        ...
        result.result[0]
          ...
          .should
          .be
          .eql('62c3b66d2d73696d706c65böm-simple')
        done()
      },
    )
  })
      ...
      (err, result) => {
        ...
        try {
          JSON.parse(result.result[0]).should.be.eql({
            ...
            currentRequest:
              `${path.resolve(fixtures, 'keys-loader.js')}??my-ident!?query`,
      ...
      (err, result) => {
        ...
        try {
          JSON.parse(result.result[0]).should.be.eql({
            ...
            currentRequest:
              `${path.resolve(fixtures, 'keys-loader.js')}??my-ident!#fragment`,
          ...
          JSON.parse(result.result[0]).should.be.eql({
            ...
            currentRequest:
              `${path.resolve(fixtures, 'keys-loader.js')
                ...
                path.resolve(fixtures, 'res\0#ource.bin')
              }?query\0#frag`,
            remainingRequest:
              `${path.resolve(fixtures, 'res\0#ource.bin')}?query\0#frag`,
            previousRequest: '',
